  ...
  stage('Clone') {
      steps {
        script {
            ...
            sh ('cd dotnet-with-k8s-jenkins && sudo dotnet publish -c Release && sudo docker build . -t docker.io/allysono/hw:latest')
            //sh ('sudo docker build . -t docker.io/allysono/hw:latest')
            sh ('sudo docker push docker.io/allysono/hw:latest')
